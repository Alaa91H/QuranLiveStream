...
for t in "${TARGETS[@]}"; do uri="$(quran_target_uri "$t" 2>/dev/null||true)"; [ -n "$uri" ]||{ log "Missing target URI: $t";exit 1;}; escaped="$(escape_tee_uri "$uri")"; TEE_SPEC="${TEE_SPEC}${TEE_SPEC:+|}[f=flv:onfail=ignore]$escaped"; TARGET_SUMMARY="${TARGET_SUMMARY}${TARGET_SUMMARY:+,}$(quran_target_redacted "$t")"; done
