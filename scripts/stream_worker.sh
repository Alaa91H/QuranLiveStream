...
# No geometry filters: Xvfb dimensions == browser viewport == encoded frame.
# ==============================================================================
set -uo pipefail
...
software_preset(){
  # Higher CPU-only tiers were benchmarked with ultrafast; never silently swap
  # in an NVENC preset name such as p4 just because ffmpeg lists the encoder.
  case "$1" in nano|micro|eco|high|ultra|extreme) echo ultrafast;; balanced) echo veryfast;; *) echo ultrafast;; esac
}

IFS=',' read -ra TARGETS <<< "$TARGET_CSV"
TARGET_COUNT=${#TARGETS[@]}; [ "$TARGET_COUNT" -gt 0 ] || exit 1
FPS="$STREAM_FPS"
for t in "${TARGETS[@]}"; do cap="$(quran_target_max_fps "$t")"; [ "$cap" -lt "$FPS" ] && FPS="$cap"; done
...
if [ "${EGRESS_GUARD:-1}" = "1" ] && [ -f "$RUNTIME/net.env" ]; then
  ...
  TOTAL_TARGETS="${STREAM_TOTAL_TARGETS:-$TARGET_COUNT}"; [[ "$TOTAL_TARGETS" =~ ^[0-9]+$ ]] || TOTAL_TARGETS="$TARGET_COUNT"
...
VAAPI_PRE=(); VF="format=yuv420p"; PIXFMT=(-pix_fmt yuv420p); VARGS=(); ACTIVE_ENCODER="cpu"
SW_PRESET="$(software_preset "$PROFILE_REQ")"
if [ "$CODEC_REQ" = "h264" ]; then
  if [ "${PREFLIGHT_HW_OK:-}" = "nvenc" ]; then
    VARGS=(-c:v h264_nvenc -preset p4 -tune hq -rc cbr -profile:v high); ACTIVE_ENCODER=nvenc
  elif [ "${PREFLIGHT_HW_OK:-}" = "vaapi" ] && [ "${HW_ALLOW_EXPERIMENTAL:-0}" = "1" ]; then
    VAAPI_PRE=(-vaapi_device /dev/dri/renderD128); VF="format=nv12,hwupload"; PIXFMT=(); VARGS=(-c:v h264_vaapi -bf 2); ACTIVE_ENCODER=vaapi
  else
    VARGS=(-c:v libx264 -preset "$SW_PRESET" -tune zerolatency -threads "$FFMPEG_THREADS" -profile:v high)
    [ -n "${X264_PARAMS:-}" ] && VARGS+=(-x264-params "$X264_PARAMS")
  fi
elif [ "$CODEC_REQ" = "h265" ] || [ "$CODEC_REQ" = "hevc" ]; then
  if ffmpeg -hide_banner -encoders 2>/dev/null | grep -q ' hevc_nvenc ' && [ "${PREFLIGHT_HW_OK:-}" = "nvenc" ]; then
    VARGS=(-c:v hevc_nvenc -preset p4 -rc cbr); ACTIVE_ENCODER=nvenc-hevc
  else
    VARGS=(-c:v libx265 -preset ultrafast -threads "$FFMPEG_THREADS"); ACTIVE_ENCODER=cpu-hevc
  fi
else
...
printf 'GROUP=%s\nLAYOUT=%s\nPROFILE=%s\nWIDTH=%s\nHEIGHT=%s\nFPS=%s\nVIDEO_KBIT=%s\nENCODER=%s\nTARGETS=%s\n' "$GROUP" "$LAYOUT" "$PROFILE_REQ" "$STREAM_WIDTH" "$STREAM_HEIGHT" "$FPS" "$VIDEO_KBIT" "$ACTIVE_ENCODER" "$TARGET_CSV" > "$GR/worker.env"

while [ ! -f "$STOP_FLAG" ]; do
  rm -f "$PROGRESS"
  log "Native encode ${STREAM_WIDTH}x${STREAM_HEIGHT}@${FPS} ${VIDEO_BITRATE} encoder=$ACTIVE_ENCODER targets=$TARGET_SUMMARY"
  set +e
  ffmpeg -hide_banner -loglevel warning -nostdin \
    ...
    "${VARGS[@]}" "${PIXFMT[@]}" -b:v "$VIDEO_BITRATE" -maxrate "$MAX_BITRATE" -bufsize "$BUF_SIZE" \
