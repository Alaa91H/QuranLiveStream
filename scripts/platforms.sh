...
quran_target_valid_name() { [[ "${1:-}" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]*$ ]]; }

quran_target_uri() {
  local target="${1:-}" prefix full_var url_var key_var full url key
  quran_target_valid_name "$target" || return 2
  prefix="$(quran_target_prefix "$target")"; full_var="${prefix}_RTMP_TARGET"; url_var="${prefix}_RTMP_URL"; key_var="${prefix}_STREAM_KEY"
  full="${!full_var:-}"
...
quran_target_host_port() {
  ...
  scheme="${uri%%://*}"; authority="${uri#*://}"; authority="${authority%%/*}"; host="$authority"; port=""
  if [[ "$authority" =~ ^\[(.*)\]:([0-9]+)$ ]]; then host="${BASH_REMATCH[1]}"; port="${BASH_REMATCH[2]}"
  elif [[ "$authority" =~ ^([^:]+):([0-9]+)$ ]]; then host="${BASH_REMATCH[1]}"; port="${BASH_REMATCH[2]}"; fi
  if [ -z "$port" ]; then case "$scheme" in rtmps) port=443;; rtmp) port=1935;; *) port=0;; esac; fi
  printf '%s %s' "$host" "$port"
}

quran_profile_rank() { case "${1:-}" in nano)echo 0;; micro)echo 1;; eco)echo 2;; balanced)echo 3;; high)echo 4;; ultra)echo 5;; extreme)echo 6;; *)echo 1;; esac; }
quran_profile_at() { case "${1:-0}" in 0)echo nano;;1)echo micro;;2)echo eco;;3)echo balanced;;4)echo high;;5)echo ultra;;*)echo extreme;; esac; }

quran_profile_dimensions() {
  local profile="${1:-micro}" layout="${2:-landscape}" w h
  case "$profile" in nano) w=640;h=360;; micro) w=854;h=480;; eco) w=1280;h=720;; balanced) w=1920;h=1080;; high) w=2560;h=1440;; ultra) w=3840;h=2160;; extreme) w=7680;h=4320;; *) w=854;h=480;; esac
  case "$layout" in portrait) printf '%s %s' "$h" "$w";; square) case "$profile" in nano)w=360;;micro)w=480;;eco)w=720;;balanced)w=1080;;high)w=1440;;ultra)w=2160;;extreme)w=4320;;esac; printf '%s %s' "$w" "$w";; *) printf '%s %s' "$w" "$h";; esac
}
...
quran_target_layout() {
  ...
  case "${target,,}" in tiktok|instagram|reels|youtube-vertical|youtube_vertical|shorts) echo portrait;; *) echo landscape;; esac
}
...
quran_target_max_profile() {
  ...
  case "${target,,}" in youtube|youtube-vertical|youtube_vertical) echo ultra;; twitch) [ "${TWITCH_ALLOW_1440P:-0}" = "1" ] && echo high || echo balanced;; kick|tiktok|instagram|facebook|reels|shorts) echo balanced;; *) echo extreme;; esac
}
...
quran_target_max_bitrate_kbit() {
  ...
  if [ -n "$value" ]; then case "$value" in *[kK])echo "${value%[kK]}";; *[mM])echo "$(( ${value%[mM]} * 1000 ))";; *[!0-9]*)echo 0;; *)echo "$value";; esac; return; fi
  case "${target,,}" in youtube|youtube-vertical|youtube_vertical) echo 40000;; kick) echo 8000;; twitch) [ "${TWITCH_ALLOW_1440P:-0}" = "1" ] && echo 7500 || echo 6000;; tiktok|instagram|facebook|reels|shorts) echo 6000;; *) echo 0;; esac
}
...
# H.264 profile is a compatibility constraint, not a quality preference.
# KICK's current ingest requirements explicitly specify Main profile; a shared
# encoder therefore uses Main whenever KICK is a member of that output group.
quran_target_h264_profile() {
  local target="${1:-}" prefix var v
  prefix="$(quran_target_prefix "$target")"; var="${prefix}_H264_PROFILE"; v="${!var:-}"
  case "${v,,}" in baseline|main|high) echo "${v,,}"; return;; esac
  case "${target,,}" in kick) echo main;; *) echo high;; esac
}
...
quran_target_audio_channels() {
  local target="${1:-}" prefix var v
  prefix="$(quran_target_prefix "$target")"; var="${prefix}_AUDIO_CHANNELS"; v="${!var:-}"
  [[ "$v" =~ ^[12]$ ]] && { echo "$v"; return; }
  case "${target,,}" in kick) echo 2;; *) echo "${AUDIO_CHANNELS:-1}";; esac
}

quran_layout_default_cities() {
  local layout="${1:-landscape}" profile="${2:-balanced}"
  case "$layout" in portrait) case "$profile" in nano|micro)echo 1;; *)echo 2;;esac;; square) case "$profile" in nano|micro)echo 2;; *)echo 3;;esac;; *) case "$profile" in nano)echo 2;;micro)echo 3;;eco)echo 5;;*)echo 6;;esac;; esac
}
...
quran_resolve_layout() {
  ...
  while IFS= read -r t; do [ -n "$t" ] || continue; current="$(quran_target_layout "$t")"; [ -z "$first" ] && first="$current"; [ "$current" != "$first" ] && mixed=1; done < <(quran_targets_lines)
