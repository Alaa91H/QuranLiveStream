...
# STREAM_PROFILE is a ceiling in universal mode, never permission to overload a weak host.
...
if [ -n "$REQUESTED_MAX_PROFILE" ]; then
  ...
  if [ "$REQUESTED_RANK" -lt "$BASE_RANK" ]; then
    BASE_RANK="$REQUESTED_RANK"; BASE_PROFILE="$(rank_profile "$BASE_RANK")"
  fi
fi
...
build_outputs(){
  ...
  for raw in "${TARGET_ARRAY[@]}"; do
    ...
    count=$((count+1)); log "TARGET ENABLED: $slug"
  done
...
apply_egress_guard(){
  [ "$EGRESS_GUARD" = "1" ] || return 0
  [ -f "$RUNTIME/net.env" ] || return 0
  ...
  [ "$safe_total" -gt 0 ] || return 0
  safe_each=$((safe_total / TARGET_COUNT))
  audio="$(to_kbit "$AUDIO_BITRATE")"; now_video="$(to_kbit "$VIDEO_BITRATE")"
  new_video=$((safe_each-audio-64))
...
read_speed(){
  local s; s="$(grep '^speed=' "$PROGRESS" 2>/dev/null | tail -1 | cut -d= -f2 | tr -d 'x' || true)"
...
adaptive_monitor(){
  ...
  while kill -0 "$ffpid" 2>/dev/null; do
    ...
    ram="$(read_ram_pct)"; speed="$(read_speed)"
    ...
    if [ "$ADAPTIVE_ENABLE" = "1" ] && [ "$ADAPTIVE_UPSHIFT" = "1" ] && [ "$under" -ge "$ADAPTIVE_UNDERLOAD_SAMPLES" ] && [ "$CURRENT_RANK" -lt "$BASE_RANK" ] && [ $((now-last_change)) -ge "$ADAPTIVE_COOLDOWN_SEC" ]; then
      new_rank=$((CURRENT_RANK+1)); [ "$new_rank" -gt "$BASE_RANK" ] && new_rank="$BASE_RANK"
...
EFFECTIVE_AUDIO="${PREFLIGHT_AUDIO:-${AUDIO_MODE:-pulse}}"
AUDIO_MODE="$EFFECTIVE_AUDIO"; export AUDIO_MODE
...
while [ ! -f "$STOP_FLAG" ]; do
  ...
  [ "$CURRENT_RANK" -gt "$BASE_RANK" ] && CURRENT_RANK="$BASE_RANK"
  load_profile "$CURRENT_RANK"; build_outputs; apply_egress_guard; rm -f "$PROGRESS"
  ...
  if [ "${PREFLIGHT_HW_OK:-}" = "nvenc" ]; then
    VARGS=(-c:v h264_nvenc -preset p4 -tune ll -rc cbr)
  elif [ "${PREFLIGHT_HW_OK:-}" = "vaapi" ] && [ "${HW_ALLOW_EXPERIMENTAL:-0}" = "1" ]; then
    VAAPI_PRE=(-vaapi_device /dev/dri/renderD128); VF="format=nv12,hwupload"
    VARGS=(-c:v h264_vaapi -bf 2)
  else
    VARGS=(-c:v libx264 -preset "$FFMPEG_PRESET" -tune "$FFMPEG_TUNE" -threads "$FFMPEG_THREADS")
